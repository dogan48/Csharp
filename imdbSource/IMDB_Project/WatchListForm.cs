...
namespace IMDB_Project
{
    public partial class WatchListForm : Form
    {
        ...
        {
            string sqlString= "SELECT * FROM Movies WHERE m_no IN (SELECT m_no FROM WatchList WHERE isWatched='false' AND u_no='" + gelenU_No.ToString() + "')";
            string sqlCount = "SELECT COUNT(*) FROM Movies WHERE m_no IN (SELECT m_no FROM WatchList WHERE isWatched='false' AND u_no='" + gelenU_No.ToString() + "')";
            ...
            watchedMovie[movieCounter] = new PictureBox();
            watchedMovie[movieCounter].Location = new Point(0, -55);
            watchedMovie[movieCounter].Size = new Size(0, 0);
            watchedMovie[movieCounter].Name = "MovieListStartPositionLabel";
            mainPanel.Controls.Add(watchedMovie[movieCounter]);
            ...
            while (connect.dr.Read())
            {
                ...
                watchedMovie[movieCounter] = new PictureBox();
                watchedMovie[movieCounter].Font = new Font("Microsoft Sans Serif", 10);
                watchedMovie[movieCounter].ForeColor = Color.Black;
                watchedMovie[movieCounter].Image = Properties.Resources.watched;
                watchedMovie[movieCounter].SizeMode = PictureBoxSizeMode.StretchImage;
                watchedMovie[movieCounter].Location = new Point(122, watchedMovie[movieCounter - 1].Location.Y + 156);
                watchedMovie[movieCounter].Size = new Size(40, 50);
                watchedMovie[movieCounter].Click += new EventHandler(watchedMovie_Click);
                //watchedMovie[movieCounter].MaximumSize = new Size(550, 85);
                watchedMovie[movieCounter].Name = connect.dr[0].ToString() + "@removeWatchListPictureBox";
                mainPanel.Controls.Add(watchedMovie[movieCounter]);
